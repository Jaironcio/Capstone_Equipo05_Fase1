// ==================== SISTEMA DE ASISTENCIA A OTRAS ACTIVIDADES ====================
class SistemaOtras extends SistemaAsistencias {
    constructor() {
        super();
        this.tipoAsistencia = 'otras';
        this.nombreClase = 'otrasSistema';
    ...
    async init() {
        ...
        document.getElementById('fechaOtras').value = new Date().toISOString().split('T')[0];
    ...
    async guardarAsistencia() {
        try {
            const fecha = document.getElementById('fechaOtras').value;
            const horaInicio = document.getElementById('horaInicio').value;
            const horaTermino = document.getElementById('horaTermino').value;
            const motivo = document.getElementById('motivoOtras').value;
            const descripcion = document.getElementById('descripcionOtras').value;

            if (!fecha) {
                Utils.mostrarNotificacion('Debe ingresar la fecha', 'error');
                return;
            }
            ...
            // Actualizar ranking
            this.actualizarRankingAsistencias(asistentes, 'otras');

            // Mensaje detallado con resumen
            const motivoTexto = motivo ? `"${motivo}"` : 'actividad';
            let mensaje = `✅ Actividad ${motivoTexto} registrada exitosamente\n\n` +
            ...
            alert(mensaje);
            Utils.mostrarNotificacion('✅ Asistencia de actividad guardada exitosamente', 'success');
...
const otrasSistema = new SistemaOtras();
