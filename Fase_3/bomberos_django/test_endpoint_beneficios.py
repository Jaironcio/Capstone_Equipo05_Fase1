...
print("="*80)
print("PROBANDO ENDPOINT BENEFICIOS API")
print("="*80)

# 1. Probar GET beneficios asignados
print("\n[1] GET /api/voluntarios/6/beneficios-asignados-simple/")
try:
    response = requests.get('http://127.0.0.1:8000/api/voluntarios/6/beneficios-asignados-simple/')
...
# 2. Probar POST pago beneficio
print("\n[2] POST /api/voluntarios/pagar-beneficio-simple/")
print("  Simulando pago de 3 tarjetas...")
try:
    # Primero obtener la asignacion_id
    response_get = requests.get('http://127.0.0.1:8000/api/voluntarios/6/beneficios-asignados-simple/')
    if response_get.status_code == 200:
        ...
        if data:
            ...
            response = requests.post(
                'http://127.0.0.1:8000/api/voluntarios/pagar-beneficio-simple/',
                json=payload
            )
